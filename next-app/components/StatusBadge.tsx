...
const icons = {
  ...
  "In Progress": (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <path d="M6 3v3l2 1.5" stroke="#7A5E00" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  ),
  "Not Started": (
    <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
      <circle cx="6" cy="6" r="5.25" stroke="#4B3A46" strokeWidth="1.5" strokeDasharray="2.5 2" />
    </svg>
  ),
};
...
export default function StatusBadge({ status }: StatusBadgeProps) {
  return (
    <span
      className={`inline-flex shrink-0 items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-semibold whitespace-nowrap ${styles[status]}`}
    >
      {icons[status]}
      {status}
    </span>
