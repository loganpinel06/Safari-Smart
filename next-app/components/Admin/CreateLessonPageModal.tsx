...
type LessonPageModalType = "Text" | "Image" | "Video";
...
export default function CreateLessonPageModal({
  ...
  const [pageType, setPageType] = useState<LessonPageModalType>("Text");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((pageType === "Image" || pageType === "Video") && (!file || file.size === 0)) {
    ...
    try {
      ...
      const response = await fetch("/api/lesson/page/create", {
        method: "POST",
        body: formData,
      });
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "Failed to create lesson page");
      }
  ...
  const imageAccept = "image/jpeg,image/jpg,image/png,image/webp,image/gif,image/svg+xml";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl bg-[#FFF1E5] shadow-lg">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto">
          ...
          <div className="flex flex-col gap-4 p-6">
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <fieldset className="flex flex-col gap-2">
                <legend className="text-sm font-semibold text-[#592803]">Type</legend>
              ...
              {pageType === "Text" ? (
                <div className="flex flex-col gap-2">
                  <label className="text-sm font-semibold text-[#592803]">Main text</label>
                  <textarea
                    value={mainText}
              ...
              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-[#592803]">
                  Sub text <span className="font-normal text-[#4B3A46]">(optional)</span>
                </label>
                <textarea
                  value={subText}
          ...
          <div className="flex flex-col gap-4 p-6">
            <div className="flex min-h-[280px] flex-1 flex-col rounded-xl border border-[#4B3A46]/15 bg-white/90 p-4 shadow-inner">
              <div className="min-h-0 flex-1 overflow-y-auto">
                {pageType === "Text" && (
                ...
                {subText.trim() ? (
                  <p className="mt-4 border-t border-[#4B3A46]/10 pt-4 text-sm leading-relaxed text-[#4B3A46] whitespace-pre-wrap">
                    {subText}
                  </p>
                ) : null}
              </div>
